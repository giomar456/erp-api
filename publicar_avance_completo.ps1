...
function Get-RemoteAssetDeletes($keepPaths) {
    $keep = @{}
    foreach ($path in $keepPaths) {
        if ($path -like "webapp/assets/*") { $keep[$path] = $true }
    }
    $tree = Invoke-GitHubJson "Get" "https://api.github.com/repos/$Owner/$Repo/git/trees/$Branch`?recursive=1"
    $deletes = @()
    foreach ($item in $tree.tree) {
        if ($item.type -ne "blob") { continue }
        if ($item.path -like "webapp/assets/*" -and -not $keep.ContainsKey($item.path)) {
            $deletes += $item.path
        }
    }
    return $deletes
...
function Publish-SingleCommit($paths, $message) {
    ...
    $treeItems = @()
    foreach ($oldAsset in (Get-RemoteAssetDeletes $paths)) {
        $treeItems += @{ path = $oldAsset; mode = "100644"; sha = $null }
        Write-Host "DEL $oldAsset" -ForegroundColor Yellow
    }
...
$commitMsg = "ERP: publicacion minima Render (api + webapp)"
Write-Host "Subiendo SOLO lo que Render necesita..." -ForegroundColor Cyan
$uploadPaths = @("api.py", "requirements-api.txt", "requirements.txt")
...
Get-ChildItem -Path (Join-Path $Root "webapp") -Recurse -File | ForEach-Object {
    ...
    if ($rel -eq "webapp/videos/serie-error.mp4") { return }
    $uploadPaths += $rel
}
